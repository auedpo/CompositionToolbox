...
function renderWorkspace2ModularGrid() {
  ...
  const list = document.createElement("div");
  list.className = "ws2-modular-grid-list";
  tracks.forEach((track, index) => {
    const row = document.createElement("div");
    row.className = "ws2-modular-grid-row";
    const laneNumber = index + 1;
    const trackName = track.name || "Untitled lane";
    const lensCount = Array.isArray(track.lensInstanceIds) ? track.lensInstanceIds.length : 0;
    row.textContent = `${laneNumber} — ${trackName} (${lensCount} lens${lensCount === 1 ? "" : "es"})`;
