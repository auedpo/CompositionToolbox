...
import {
  WS2_GRID_INITIAL_LANES,
  WS2_GRID_ROWS,
  clampWs2Row,
  isWs2RowInRange,
  getDefaultLaneName
} from "./workspace2GridConfig.js";
...
function getWorkspace2Els() {
  ...
  return {
    ...
    modularGrid: document.getElementById("ws2ModularGrid"),
...
const ws2GridDragHighlight = { cell: null, lane: null };
let ws2GridOccupancySnapshot = null;
let ws2GridHintElement = null;
const WS2_GRID_HINT_DEFAULT = "Drop lenses into empty grid slots.";
...
function beginWs2InstanceDrag(lensInstanceId, laneId, row) {
  ...
  state.ws2Drag = {
    ...
    lensId: null,
    lensInstanceId,
    fromLaneId: laneId || null,
    fromRow: Number.isFinite(row) ? row : null
  };
...
function getWs2DragPayload() {

  return state.ws2Drag?.active ? state.ws2Drag : null;

}



function resetWs2GridDragHighlights() {

  document.querySelectorAll(".ws2-grid-cell.is-valid-drop, .ws2-grid-cell.is-invalid-drop").forEach((el) => {
    el.classList.remove("is-valid-drop", "is-invalid-drop");
  });

  document.querySelectorAll(".ws2-modular-grid-lane--drop-target, .ws2-modular-grid-lane--full-warning").forEach((el) => {
    el.classList.remove("ws2-modular-grid-lane--drop-target", "ws2-modular-grid-lane--full-warning");
  });

  ws2GridDragHighlight.cell = null;
  ws2GridDragHighlight.lane = null;

  if (ws2GridHintElement) {
    ws2GridHintElement.textContent = WS2_GRID_HINT_DEFAULT;
  }
...
function endWs2GridDrag({ render = true } = {}) {
...
function getWs2GridOccupant(laneId, row) {

  if (!laneId || !Number.isFinite(row)) return null;

  const snapshot = ws2GridOccupancySnapshot;

  if (snapshot?.occupancy?.has(laneId)) {
    const laneCells = snapshot.occupancy.get(laneId);
    if (Array.isArray(laneCells) && row >= 0 && row < laneCells.length) {
      return laneCells[row] || null;
    }
  }

  const track = getTrackById(laneId);

  if (!track) return null;

  for (const instanceId of ensureTrackLensOrder(track)) {
    const instance = lensInstances.get(instanceId);
    if (!instance) continue;
    if (Number.isFinite(instance.row) && instance.row === row) {
      return instanceId;
    }
  }
...
function getLaneOccupancyCount(laneId) {

  if (!laneId) return 0;

  const snapshot = ws2GridOccupancySnapshot;

  if (snapshot?.laneCounts?.has(laneId)) {
    return snapshot.laneCounts.get(laneId) || 0;
  }
  ...
  ensureTrackLensOrder(track).forEach((instanceId) => {
    const instance = lensInstances.get(instanceId);
    if (instance && Number.isFinite(instance.row) && instance.row >= 0 && instance.row < WS2_GRID_ROWS) {
      count += 1;
    }
  });
...
function getWs2GridDropValidity(payload, laneId, row) {

  if (!payload || !laneId || !Number.isFinite(row)) {
    return { valid: false, reason: "invalid" };
  }

  if (!isWs2RowInRange(row)) {
    return { valid: false, reason: "invalid-row" };
  }

  const occupantId = getWs2GridOccupant(laneId, row);
  const laneCount = getLaneOccupancyCount(laneId);
  const laneFull = laneCount >= WS2_GRID_ROWS;

  if (payload.type === "lensDef") {
    if (laneFull) return { valid: false, reason: "lane-full" };
    if (occupantId) return { valid: false, reason: "occupied" };
    return { valid: true };
  }

  if (payload.type === "lensInstance") {
    if (!payload.lensInstanceId) return { valid: false, reason: "invalid" };
    if (occupantId) return { valid: false, reason: "occupied" };
    return { valid: true };
  }
...
function applyWs2GridHighlight(cell, laneEl, validity) {

  if (!cell) return;

  if (ws2GridDragHighlight.cell && ws2GridDragHighlight.cell !== cell) {
    ws2GridDragHighlight.cell.classList.remove("is-valid-drop", "is-invalid-drop");
  }

  ws2GridDragHighlight.cell = cell;
  cell.classList.toggle("is-valid-drop", !!validity.valid);
  cell.classList.toggle("is-invalid-drop", !validity.valid);

  if (ws2GridDragHighlight.lane && ws2GridDragHighlight.lane !== laneEl) {
    ws2GridDragHighlight.lane.classList.remove("ws2-modular-grid-lane--drop-target", "ws2-modular-grid-lane--full-warning");
  }

  ws2GridDragHighlight.lane = laneEl;

  if (laneEl) {
    laneEl.classList.toggle("ws2-modular-grid-lane--drop-target", validity.valid);
    laneEl.classList.toggle("ws2-modular-grid-lane--full-warning", validity.reason === "lane-full");
  }

  if (ws2GridHintElement) {
    ws2GridHintElement.textContent =
      validity.reason === "lane-full"
        ? `${laneEl?.dataset.laneLabel ?? "Lane"} is full (${WS2_GRID_ROWS}/${WS2_GRID_ROWS})`
        : WS2_GRID_HINT_DEFAULT;
  }
...
function handleWs2GridCellDragOver(event) {

  const payload = getWs2DragPayload();
  ...
  if (!laneId || !Number.isFinite(row)) return;

  event.preventDefault();

  if (event.dataTransfer) {
    event.dataTransfer.dropEffect = payload.type === "lensInstance" ? "move" : "copy";
  }

  const validity = getWs2GridDropValidity(payload, laneId, row);
  const laneEl = cell.closest(".ws2-modular-grid-lane");
  applyWs2GridHighlight(cell, laneEl, validity);

}



function handleWs2GridCellDrop(event) {
  ...
  if (!laneId || !Number.isFinite(row)) {
    endWs2GridDrag({ render: true });
    return;
  }
  ...
  if (validity.valid) {
    if (payload.type === "lensDef") {
      createLensInstanceForTrack(payload.lensId, laneId, null, { row, skipRender: true });
    } else if (payload.type === "lensInstance") {
      moveLensInstanceToCell({
        lensInstanceId: payload.lensInstanceId,
        toLaneId: laneId,
        toRow: row
      });
    }
  } else if (validity.reason === "lane-full" && ws2GridHintElement) {
    ws2GridHintElement.textContent = `${laneEl?.dataset.laneLabel ?? "Lane"} is full (${WS2_GRID_ROWS}/${WS2_GRID_ROWS})`;
  }
...
function buildWorkspace2Occupancy(tracks) {
  ...
  if (!Array.isArray(tracks)) return snapshot;

  tracks.forEach((track) => {

    const laneId = track && track.id ? track.id : null;

    if (!laneId) return;

    const cells = Array.from({ length: WS2_GRID_ROWS }, () => null);

    let count = 0;

    const lensIds = Array.isArray(track.lensInstanceIds) ? track.lensInstanceIds : [];

    lensIds.forEach((lensInstanceId) => {

      if (!lensInstanceId) return;

      const instance = lensInstances.get(lensInstanceId);

      const row = Number.isFinite(instance?.row) ? instance.row : -1;

      if (row >= 0 && row < WS2_GRID_ROWS && !cells[row]) {

        cells[row] = lensInstanceId;
    ...
    snapshot.occupancy.set(laneId, cells);

    snapshot.laneCounts.set(laneId, count);
...
function reorderTrackLensIdsByRow(track) {
  ...
  const next = lensIds
    ...
    .sort((a, b) => {
      ...
      const rowA = Number.isFinite(lensInstances.get(a)?.row) ? lensInstances.get(a).row : Number.POSITIVE_INFINITY;

      const rowB = Number.isFinite(lensInstances.get(b)?.row) ? lensInstances.get(b).row : Number.POSITIVE_INFINITY;
...
function ensureWorkspace2Tracks() {

  const ordered = getOrderedTracks();

  let created = false;

  while (ordered.length < WS2_GRID_INITIAL_LANES) {

    const laneIndex = ordered.length;

    const laneName = getDefaultLaneName(laneIndex);

    const track = createTrack(laneName);
...
function createLensInstanceForTrack(lensId, trackId, insertIndex = null, options = {}) {
  ...
  const { row: requestedRow = null, skipRender = false } = normalizedOptions || {};
  ...
  const normalizedRow = clampWs2Row(requestedRow);

  if (Number.isFinite(normalizedRow)) {

    instance.row = normalizedRow;
  ...
  reorderTrackLensIdsByRow(track);
...
function moveLensInstanceToCell({ lensInstanceId, toLaneId, toRow }) {

  if (!lensInstanceId || !toLaneId || !isWs2RowInRange(toRow)) return false;

  const instance = lensInstances.get(lensInstanceId);

  if (!instance) return false;

  const targetTrack = getTrackById(toLaneId);

  if (!targetTrack) return false;

  const occupantId = getWs2GridOccupant(toLaneId, toRow);

  if (occupantId) return false;

  const sourceTrack = getTrackById(instance.trackId);

  if (sourceTrack) {

    sourceTrack.lensInstanceIds = ensureTrackLensOrder(sourceTrack).filter((id) => id !== lensInstanceId);

    reorderTrackLensIdsByRow(sourceTrack);

    updateTrackLensPaths(sourceTrack);

  }

  const normalizedRow = clampWs2Row(toRow);
  ...
  instance.trackId = toLaneId;

  targetTrack.lensInstanceIds = ensureTrackLensOrder(targetTrack).concat(lensInstanceId);

  reorderTrackLensIdsByRow(targetTrack);

  updateTrackLensPaths(targetTrack);
  ...
  [sourceTrack, targetTrack].forEach((track) => {

    if (!track) return;

    ensureTrackLensOrder(track).forEach((id) => {

      if (scheduled.has(id)) return;

      scheduled.add(id);

      const lens = lensInstances.get(id);
...
function renderWorkspace2ModularGrid() {
  const ws2 = getWorkspace2Els();
  if (!ws2.modularGrid) return;
  ensureWorkspace2Tracks();
  const panel = ws2.modularGrid;
  const body = panel.querySelector('.ws2-panel-body') || panel;
  resetWs2GridDragHighlights();
  ws2GridHintElement = null;
  const tracks = getOrderedTracks();
  if (!tracks.length) {
    body.innerHTML = `<div class="ws2-placeholder">Modular grid placeholder will show lanes once they exist.</div>`;
    return;
  }
  body.innerHTML = '';
  const gridRoot = document.createElement('div');
  gridRoot.className = 'ws2-modular-grid';
  gridRoot.style.setProperty('--ws2-lane-count', tracks.length.toString());
  gridRoot.style.setProperty('--ws2-row-count', WS2_GRID_ROWS.toString());
  const hint = document.createElement('div');
  hint.className = 'ws2-modular-grid-hint';
  hint.textContent = WS2_GRID_HINT_DEFAULT;
  ws2GridHintElement = hint;
  gridRoot.appendChild(hint);
  const header = document.createElement('div');
  header.className = 'ws2-modular-grid-header';
  tracks.forEach((track, index) => {
    const laneLabel = track.name || getDefaultLaneName(index);
    const headerCell = document.createElement('div');
    headerCell.className = 'ws2-modular-grid-header-cell';
    headerCell.textContent = `${index + 1}. ${laneLabel}`;
    header.appendChild(headerCell);
  });
  gridRoot.appendChild(header);
  const occupancy = buildWorkspace2Occupancy(tracks);
  ws2GridOccupancySnapshot = occupancy;
  const laneContainer = document.createElement('div');
  laneContainer.className = 'ws2-modular-grid-lanes';
  tracks.forEach((track, laneIndex) => {
    const laneId = track && track.id ? track.id : '';
    const laneLabel = track && track.name ? track.name : getDefaultLaneName(laneIndex);
    const occupiedCount = laneId ? occupancy.laneCounts.get(laneId) || 0 : 0;
    const laneColumn = document.createElement('div');
    laneColumn.className = 'ws2-modular-grid-lane';
    if (occupiedCount >= WS2_GRID_ROWS) {
      laneColumn.classList.add('ws2-modular-grid-lane--full');
    }
    laneColumn.dataset.laneId = laneId;
    laneColumn.dataset.laneLabel = laneLabel;
    laneColumn.dataset.laneIndex = laneIndex.toString();
    const laneHeader = document.createElement('div');
    laneHeader.className = 'ws2-modular-grid-lane-header';
    const title = document.createElement('span');
    title.className = 'ws2-modular-grid-lane-title';
    title.textContent = laneLabel;
    const count = document.createElement('span');
    count.className = 'ws2-modular-grid-lane-count';
    count.textContent = `${occupiedCount}/${WS2_GRID_ROWS}`;
    laneHeader.appendChild(title);
    laneHeader.appendChild(count);
    laneColumn.appendChild(laneHeader);
    const laneBody = document.createElement('div');
    laneBody.className = 'ws2-modular-grid-lane-body';
    laneBody.style.setProperty('--ws2-row-count', WS2_GRID_ROWS.toString());
    const laneOccupancy = laneId ? occupancy.occupancy.get(laneId) || [] : [];
    for (let row = 0; row < WS2_GRID_ROWS; row += 1) {
      const cell = document.createElement('div');
      cell.className = 'ws2-grid-cell';
      cell.dataset.laneId = laneId;
      cell.dataset.row = row.toString();
      cell.setAttribute('role', 'button');
      cell.setAttribute('tabindex', '-1');
      cell.title = `Lane ${laneIndex + 1} - Row ${row}`;
      const occupantId = laneOccupancy[row] || null;
      if (occupantId) {
        const instance = lensInstances.get(occupantId);
        if (instance) {
          cell.classList.add('is-occupied');
          const node = document.createElement('div');
          node.className = 'ws2-modular-grid-node';
          node.setAttribute('draggable', 'true');
          node.dataset.lensInstanceId = instance.lensInstanceId;
          node.tabIndex = 0;
          const label = document.createElement('div');
          label.className = 'ws2-modular-grid-node-label';
          label.textContent = instance.lens?.meta?.name || instance.lensId || 'Lens';
          const meta = document.createElement('div');
          meta.className = 'ws2-modular-grid-node-meta';
          meta.textContent = `Lane ${laneIndex + 1} - Row ${row}`;
          ...
          node.addEventListener('dragstart', (event) => {
            beginWs2InstanceDrag(instance.lensInstanceId, laneId, row);
            if (event.dataTransfer) {
              event.dataTransfer.setData('application/x-lens-instance', instance.lensInstanceId);
              event.dataTransfer.setData('text/plain', instance.lensInstanceId);
      ...
      cell.addEventListener('dragenter', handleWs2GridCellDragOver);
      cell.addEventListener('dragover', handleWs2GridCellDragOver);
      cell.addEventListener('drop', handleWs2GridCellDrop);
      laneBody.appendChild(cell);
    }
    laneColumn.appendChild(laneBody);
    laneContainer.appendChild(laneColumn);
  });
...
function renderWorkspace2LensInspector() {
  ...
  const describeLaneName = (laneId) => {

    if (!laneId) return "Lane";

    const laneTrack = getTrackById(laneId);

    const laneNumber = laneTrack ? getTrackNumber(laneTrack.id) : 0;

    const laneLabel = laneTrack && laneTrack.name ? laneTrack.name : "Untitled lane";

    return laneNumber ? `Track ${laneNumber} — ${laneLabel}` : laneLabel;
  ...
  if (!inputSpecs.length) {
    ...
    inputSpecs.forEach((spec) => {
      ...
      const laneSelection = inst.selectedInputLaneByRole && typeof inst.selectedInputLaneByRole === "object"

        ? inst.selectedInputLaneByRole[spec.role]

        : null;

      const laneValue = typeof laneSelection === "string" ? laneSelection : "auto";

      const block = document.createElement("div");

      block.className = "ws2-inspector-wiring-block";

      const headerRow = document.createElement("div");

      headerRow.className = "ws2-inspector-wiring-header";

      const roleLabel = document.createElement("div");

      roleLabel.className = "ws2-inspector-wiring-role";

      roleLabel.textContent = spec.label || spec.role || "Input";

      const selectEl = document.createElement("select");

      selectEl.className = "ws2-inspector-lane-select";

      selectEl.dataset.role = spec.role;
      ...
      appendOption("auto", "Auto (this lane)");
      ...
      orderedTracks.forEach((track, index) => {
        ...
        const laneName = track.name || "Untitled lane";

        appendOption(track.id, `${laneNumber} — ${laneName}`);

      });

      const hasValue = Array.from(selectEl.options).some((option) => option.value === laneValue);

      selectEl.value = hasValue ? laneValue : "auto";

      const isFrozen = isFreezeRef((inst.selectedInputRefsByRole || {})[spec.role]);

      if (isFrozen) {

        selectEl.disabled = true;

        selectEl.title = "Frozen input reference";

      }

      headerRow.appendChild(roleLabel);

      headerRow.appendChild(selectEl);

      if (isFrozen) {

        const frozenBadge = document.createElement("span");

        frozenBadge.className = "ws2-inspector-lane-frozen";

        frozenBadge.textContent = "Frozen";

        headerRow.appendChild(frozenBadge);
      ...
      statusLine.className = "ws2-inspector-lane-status";

      const targetLaneId = track ? track.id : null;

      const targetRow = Number.isFinite(inst.row) ? inst.row : Number.MAX_SAFE_INTEGER;

      const targetRowLabel = Number.isFinite(inst.row) ? inst.row : "current position";

      const resolvedLaneId = resolveSourceLaneId({

        index: laneIndex,

        targetLaneId,

        selection: selectEl.value || "auto"

      });

      const laneLabel = describeLaneName(resolvedLaneId);

      const upstreamLensInstanceId = findNearestUpstreamLens({

        index: laneIndex,

        sourceLaneId: resolvedLaneId,
      ...
      if (upstreamLensInstanceId) {
        ...
        const upstreamInstance = lensInstances.get(upstreamLensInstanceId);

        let upstreamRow = laneIndex.lensToRow

          ? laneIndex.lensToRow.get(upstreamLensInstanceId)

          : null;

        if (!Number.isFinite(upstreamRow) && upstreamInstance && Number.isFinite(upstreamInstance.row)) {

          upstreamRow = upstreamInstance.row;

        }

        const laneTrack = resolvedLaneId ? getTrackById(resolvedLaneId) : null;

        const laneTrackNumber = laneTrack ? getTrackNumber(laneTrack.id) : 0;

        const upstreamIndex = laneTrack

          ? getLensIndexInTrack(laneTrack, upstreamLensInstanceId)

          : -1;

        const lensLabel = getLensLabelForTrackIndex(laneTrackNumber, upstreamIndex);

        const upstreamDisplay = lensLabel

          || (upstreamInstance && upstreamInstance.lens && upstreamInstance.lens.meta && upstreamInstance.lens.meta.name)

          || "Lens";

        const rowText = Number.isFinite(upstreamRow) ? upstreamRow : "—";

        statusLine.textContent = `Resolved: ${laneLabel} → ${upstreamDisplay} (row ${rowText})`;

      } else {

        statusLine.textContent = `No upstream in ${laneLabel} above row ${targetRowLabel}`;
      ...
      selectEl.addEventListener("change", (event) => {
        ...
        const hasLane = tracks.some((candidate) => candidate.id === value);

        const nextValue = value === "auto" || hasLane ? value : "auto";

        inst.selectedInputLaneByRole = inst.selectedInputLaneByRole || {};

        inst.selectedInputLaneByRole[spec.role] = nextValue;
